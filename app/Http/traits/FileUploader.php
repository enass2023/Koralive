...
trait FileUploader
{
    ...
    public function uploadImagePublic3($request, $name,$inputName='images',$keyname)
    {
        $requestFile = request($inputName)[$keyname];
        try {
            $dir = 'storage/images/'.$name;
            // $fixName = $data->id.'-'.$name.'.'.$requestFile->extension();
    ...
    public function uploadImagePublic2($request, $name,$inputName='image')
    {
        ...
        try {
            ...
            if ($requestFile) {
                  Storage::putFileAs($dir, $requestFile, $fixName);
                $request->image = $fixName;
                $url='storage/images/'.$name.'/'.$fixName;
            }
            return $url;
        } catch (\Throwable $th) {
            report($th);
    ...
    public function uploadMultiImage2($request, $name, $inputName = 'images')
    {
        // Ensure the request has the files for the given input name
        // if (!$request->hasFile($inputName)) {
        //     return ['status' => 'Error', 'message' => 'No files found for the input name: ' . $inputName];
        ...
        $requestFiles = $request->file($inputName);
       $requestFilesValues= array_values($requestFiles);
        // Check if the input is an array
        // var_dump($requestFiles);
        if (!is_array($requestFiles)) {
            return ['status' => 'Error', 'message' => 'The input must be an array of files for: ' . $inputName];
        ...
        foreach ($requestFiles as $key=>$file) {
            $rand=Str::random(10);
            $dir = 'images/' . $name;
            $fixName = $rand. '-' . $name . '.'  . $file->getClientOriginalExtension();

            if ($file) {
               $url= Storage::putFileAs($dir, $file, $fixName);
                
                
                $requestFiles[$key] =   $dir . '/' . $fixName;
            }
        }
